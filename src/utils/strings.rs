...
lazy_static! {
	static ref SPLIT_UPPER_LOWER: Regex = Regex::new(r"(?x) (\p{Upper}+)(\p{Upper}\p{Lower})").unwrap();
	static ref SPLIT_LOWER_UPPER: Regex = Regex::new(r"(?x) ([\p{Lower}\d])(\p{Upper})").unwrap();
...
impl StringExt<String> for String {
	...
	fn titleize(self) -> String {
		...
		string = LOWER_STARTS
			.replace_all(&string, |caps: &Captures| caps[0].to_uppercase())
			.to_string();
		string
	}
